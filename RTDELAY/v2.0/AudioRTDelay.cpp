...
bool AudioRTDelay::initialize(void)
{
	...
	this->BUFFER_SIZE_SAMPLES = (this->N_CHANNELS)*(this->BUFFER_SIZE_FRAMES);
	...
	this->BUFFER_SEGMENT_SIZE_SAMPLES = (this->N_CHANNELS)*(this->BUFFER_SEGMENT_SIZE_FRAMES);
...
bool AudioRTDelay::runDSP(size_t n_segment)
{
	...
	for(n_frame = 0u; n_frame < this->BUFFER_SEGMENT_SIZE_FRAMES; n_frame++)
	{
		...
		{
			p_curr_seg_out[n_currsample] = p_curr_seg_in[n_currsample];
			n_currsample++;
		}
...
bool AudioRTDelay::buffer_alloc(void)
{
	...
	{
		this->pp_bufferinput_segments[n_seg] = &(this->p_bufferinput[n_seg*(this->BUFFER_SEGMENT_SIZE_SAMPLES)]);
		this->pp_bufferoutput_segments[n_seg] = &(this->p_bufferoutput[n_seg*(this->BUFFER_SEGMENT_SIZE_SAMPLES)]);
	}
