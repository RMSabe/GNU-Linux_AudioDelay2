...
bool AudioStream::initialize(void)
{
	...
	if(this->N_CHANNELS < this->N_CHANNELS_MIN)
	{
		...
		this->err_msg = __TEXT("AudioStream::initialize: Error: invalid audio channel count.");
		return false;
	}
...
bool AudioStream::loadAudioDeviceList(bool output)
{
	...
	audiodevicelist_entry_t *p_devlist = NULL;
	size_t *p_devlist_n_entries = NULL;
	...
	if(output)
	{
		this->audiodevicelist_free(&(this->p_audiooutputdevicelist));
		p_devlist_n_entries = &(this->audiooutputdevicelist_n_entries);
	}
	else
	{
		this->audiodevicelist_free(&(this->p_audioinputdevicelist));
		p_devlist_n_entries = &(this->audioinputdevicelist_n_entries);
	}

	*p_devlist_n_entries = 0u;
	...
	while(n_sndctl < n_sndctl_count)
	{
		...
		while(pp_hints[n_hint] != NULL)
		{
			...
			if(_cstr_char_compare_upto_char(p_devname, "hw", ':', true) || _cstr_char_compare_upto_char(p_devname, "plughw", ':', true))
			{
				...
				if(ioid_matched) (*p_devlist_n_entries)++;
	...
	if(!(*p_devlist_n_entries))
	{
		this->err_msg = __TEXT("AudioStream::loadAudioDeviceList: Error: no devices found.");
		return false;
	}

	if(!this->audiodevicelist_alloc(&p_devlist, *p_devlist_n_entries))
	{
		this->status = this->STATUS_ERROR_MEMORY;
		return false;
	}

	if(output) this->p_audiooutputdevicelist = p_devlist;
	else this->p_audioinputdevicelist = p_devlist;
		...
		while(pp_hints[n_hint] != NULL)
		{
			...
			if(_cstr_char_compare_upto_char(p_devname, "hw", ':', true) || _cstr_char_compare_upto_char(p_devname, "plughw", ':', true))
			{
				...
				if(ioid_matched)
				{
					...
					p_audiodevicelist_entry_name = ((audiodevicelist_entry_t*) (((uintptr_t) p_devlist) + audiodevicelist_byteindex))->name;
					p_audiodevicelist_entry_desc = ((audiodevicelist_entry_t*) (((uintptr_t) p_devlist) + audiodevicelist_byteindex))->desc;
...
{
	if(output) return (ssize_t) this->audiooutputdevicelist_n_entries;

	return (ssize_t) this->audioinputdevicelist_n_entries;
}

const audiodevicelist_entry_t* AudioStream::getAudioDeviceListEntry(size_t index, bool output)
{
	audiodevicelist_entry_t *p_devlist = NULL;
	size_t devlist_n_entries;

	if(output)
	{
		p_devlist = this->p_audiooutputdevicelist;
		devlist_n_entries = this->audiooutputdevicelist_n_entries;
	}
	else
	{
		p_devlist = this->p_audioinputdevicelist;
		devlist_n_entries = this->audioinputdevicelist_n_entries;
	}
	...
	if(index >= devlist_n_entries)
	{
		this->err_msg = __TEXT("AudioStream::getAudioDeviceListEntry: Error: given index is out of bounds.");
		return NULL;
	}

	return (const audiodevicelist_entry_t*) (((uintptr_t) p_devlist) + index*sizeof(audiodevicelist_entry_t));
}
...
bool AudioStream::chooseDevice(size_t index, bool output)
{
	...
	*p_audiodevdesc = p_devlist[index].name;
	return true;
}
...
bool AudioStream::audiodevicelist_alloc(audiodevicelist_entry_t **pp_devlist, size_t n_entries)
{
	if(pp_devlist == NULL) return false;

	this->audiodevicelist_free(pp_devlist);

	if(!n_entries)
	{
		this->err_msg = __TEXT("AudioStream::audiodevicelist_alloc: Error: list has no entries.");
		return false;
	}

	*pp_devlist = (audiodevicelist_entry_t*) malloc(n_entries*sizeof(audiodevicelist_entry_t));
	if(*pp_devlist == NULL)
	{
		this->status = this->STATUS_ERROR_MEMORY;
		this->err_msg = __TEXT("AudioStream::audiodevicelist_alloc: Error: memory allocate failed.");
		return false;
	}

	memset(*pp_devlist, 0, (n_entries*sizeof(audiodevicelist_entry_t)));
	return true;
}

void AudioStream::audiodevicelist_free(audiodevicelist_entry_t **pp_devlist)
{
	if(pp_devlist == NULL) return;
	if(*pp_devlist == NULL) return;
