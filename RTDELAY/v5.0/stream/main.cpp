 ...
 * Version 5.0 (Audio Stream Version)
 *
 * Author: Rafael Sabe
...
#define __AUDIOSTREAM_STREAMBUFFER_SEGMENT_SIZE_FRAMES 4096U
#define __AUDIOSTREAM_STREAMBUFFER_N_SEGMENTS 4U
#define __AUDIOSTREAM_DELAY_BUFFER_SIZE_FRAMES 65536U
#define __AUDIOSTREAM_DELAY_N_FFCH 4U
#define __AUDIOSTREAM_DELAY_N_FBCH 4U
...
int main(int argc, char **argv)
{
	if(argc < 6)
	{
		__STDCOUT__ << __TEXT("Error: missing arguments\nThis executable requires 5 arguments: <input device id> <output device id> <sampling rate> <bit depth> <number of channels>\nThey must be in that order\n");
		return -1;
	}
	...
	stream_params.audiobuffer_size_frames = _get_closest_power2_ceil(stream_params.sample_rate);
	stream_params.streambuffer_segment_size_frames = __AUDIOSTREAM_STREAMBUFFER_SEGMENT_SIZE_FRAMES;
	stream_params.streambuffer_n_segments = __AUDIOSTREAM_STREAMBUFFER_N_SEGMENTS;
	stream_params.delay_buffer_size_frames = __AUDIOSTREAM_DELAY_BUFFER_SIZE_FRAMES;
	stream_params.delay_n_ff_delays = __AUDIOSTREAM_DELAY_N_FFCH;
	stream_params.delay_n_fb_delays = __AUDIOSTREAM_DELAY_N_FBCH;
	...
	p_audio->chooseDevice(audiodevdesc_in, false);
	p_audio->chooseDevice(audiodevdesc_out, true);
...
static void cmdui_cmd_decode(void)
{
	...
	if(cstr_compare_upto_len(__TEXT("setffdelay"), cmd, 10u, false))
	{
		...
		n_fx = cmdui_parse_nfx(textbuf, (ssize_t) __AUDIOSTREAM_DELAY_N_FFCH);
		if(n_fx < 0) return;
	...
	if(cstr_compare_upto_len(__TEXT("setfbdelay"), cmd, 10u, false))
	{
		...
		n_fx = cmdui_parse_nfx(textbuf, (ssize_t) __AUDIOSTREAM_DELAY_N_FBCH);
		if(n_fx < 0) return;
...
static void cmdui_print_help_text(void)
{
	...
	__STDCOUT__ << __TEXT("\"setffdelay<x>:<number>\" : set the delay time (in number of samples) for a specific feedforward delay channel. \"number\" is the delay time and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOSTREAM_DELAY_N_FFCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setffamp<x>:<number>\" : set the amplitude for a specific feedforward delay channel. \"number\" is the amplitude and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOSTREAM_DELAY_N_FFCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setfbdelay<x>:<number>\" : set the delay time (in number of samples) for a specific feedback delay channel. \"number\" is the delay time and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOSTREAM_DELAY_N_FBCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setfbamp<x>:<number>\" : set the amplitude for a specific feedback delay channel. \"number\" is the amplitude and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOSTREAM_DELAY_N_FBCH) << __TEXT(")\n\n");
...
static void cmdui_print_current_params(void)
{
	...
	n_fx = 0u;
	while(n_fx < __AUDIOSTREAM_DELAY_N_FFCH)
	{
	...
	n_fx = 0u;
	while(n_fx < __AUDIOSTREAM_DELAY_N_FBCH)
	{
...
static ssize_t cmdui_parse_ndelay(const __tchar_t *text_ndelay)
{
	...
	if(n_delay >= ((long) __AUDIOSTREAM_DELAY_BUFFER_SIZE_FRAMES))
	{
