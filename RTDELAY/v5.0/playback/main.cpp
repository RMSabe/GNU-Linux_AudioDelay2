 ...
 * Version 5.0 (File Playback Version)
 *
 * Author: Rafael Sabe
...
#define __AUDIOPB_DELAY_N_FFCH 4U
#define __AUDIOPB_DELAY_N_FBCH 4U
...
int main(int argc, char **argv)
{
	...
	pb_params.delay_n_ff_delays = __AUDIOPB_DELAY_N_FFCH;
	pb_params.delay_n_fb_delays = __AUDIOPB_DELAY_N_FBCH;
...
static void cmdui_cmd_decode(void)
{
	...
	if(cstr_compare_upto_len(__TEXT("setffdelay"), cmd, 10u, false))
	{
		...
		n_fx = cmdui_parse_nfx(textbuf, (ssize_t) __AUDIOPB_DELAY_N_FFCH);
		if(n_fx < 0) return;
	...
	if(cstr_compare_upto_len(__TEXT("setfbdelay"), cmd, 10u, false))
	{
		...
		n_fx = cmdui_parse_nfx(textbuf, (ssize_t) __AUDIOPB_DELAY_N_FBCH);
		if(n_fx < 0) return;
...
static void cmdui_print_help_text(void)
{
	...
	__STDCOUT__ << __TEXT("\"setffdelay<x>:<number>\" : set the delay time (in number of samples) for a specific feedforward delay channel. \"number\" is the delay time and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOPB_DELAY_N_FFCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setffamp<x>:<number>\" : set the amplitude for a specific feedforward delay channel. \"number\" is the amplitude and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOPB_DELAY_N_FFCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setfbdelay<x>:<number>\" : set the delay time (in number of samples) for a specific feedback delay channel. \"number\" is the delay time and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOPB_DELAY_N_FBCH) << __TEXT(")\n");
	__STDCOUT__ << __TEXT("\"setfbamp<x>:<number>\" : set the amplitude for a specific feedback delay channel. \"number\" is the amplitude and \"x\" is the delay channel. (valid x values are 1 to ") << __TOSTRING(__AUDIOPB_DELAY_N_FBCH) << __TEXT(")\n\n");
...
static void cmdui_print_current_params(void)
{
	...
	n_fx = 0u;
	while(n_fx < __AUDIOPB_DELAY_N_FFCH)
	{
	...
	n_fx = 0u;
	while(n_fx < __AUDIOPB_DELAY_N_FBCH)
	{
