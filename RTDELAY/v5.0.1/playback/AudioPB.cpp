...
bool AudioPB::chooseDevice(size_t index)
{
	...
	this->AUDIODEV_DESC = this->audiodevlist.p_entries[index].name;
	return true;
}
...
bool AudioPB::audio_hw_init(void)
{
	...
	n_ret = snd_pcm_hw_params_set_format(this->p_audiodev, p_hwparams, (snd_pcm_format_t) this->AUDIODEV_FORMAT);
	if(n_ret < 0)
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
	...
	n_ret = snd_pcm_hw_params_set_period_size(this->p_audiodev, p_hwparams, (snd_pcm_uframes_t) this->STREAMBUFFER_SEGMENT_SIZE_FRAMES, 0);
	if(n_ret < 0)
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
...
void AudioPB::buffer_play(void)
{
	...
	n_ret = (ssize_t) snd_pcm_writei(this->p_audiodev, p_playout, (snd_pcm_uframes_t) this->STREAMBUFFER_SEGMENT_SIZE_FRAMES);
	if(n_ret < 0)
	{
		if(n_ret == -EPIPE)
		{
