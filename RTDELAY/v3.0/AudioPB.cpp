...
bool AudioPB::setParameters(const audiopb_params_t *p_params)
{
	if(this->status > 0)
	{
		this->err_msg = "AudioPB::setParameters: Error: AudioPB object is already initialized.";
		return false;
	}
	...
	if(p_params == NULL)
	{
		this->err_msg = "AudioPB::setParameters: Error: given p_params object pointer is NULL.";
		return false;
	}

	if(p_params->file_dir == NULL)
	{
		this->err_msg = "AudioPB::setParameters: Error: p_params: given file_dir value is invalid.";
		return false;
	}

	if(p_params->audio_dev_desc == NULL)
	{
		this->err_msg = "AudioPB::setParameters: Error: p_params: given audio_dev_desc value is invalid.";
		return false;
	}
...
bool AudioPB::runPlayback(void)
{
	...
	{
		this->err_msg = "AudioPB::runPlayback: Error: AudioPB object is either not initialized or already playing.";
		return false;
	}
...
void AudioPB::buffer_play(void)
{
	...
	if(n_ret < 0)
	{
		if(n_ret == -EPIPE)
		{
			...
			if(n_ret < 0) app_exit(1, "CRITICAL ERROR OCCURRED: AudioPB::buffer_play: Error: snd_pcm_prepare failed.");
		}
		else app_exit(1, "CRITICAL ERROR OCCURRED: AudioPB::buffer_play: Error: snd_pcm_writei failed.");
	}
...
void AudioPB::cmdui_cmd_decode(void)
{
	...
	if(cstr_compare("resetff", cmd))
	{
		this->p_delay->resetFFParams();
		return;
	}

	if(cstr_compare("resetfb", cmd))
	{
		this->p_delay->resetFBParams();
		return;
	}

	if(this->cmdui_cmd_compare("setdryamp:", cmd, 10u))
	{
		this->cmdui_attempt_update_dryamp(&cmd[10u]);
		return;
	}

	if(this->cmdui_cmd_compare("setoutamp:", cmd, 10u))
	{
		this->cmdui_attempt_update_outamp(&cmd[10u]);
		return;
	}
...
void AudioPB::cmdui_print_help_text(void)
{
	...
	std::cout << "\"reset\" to reset all feedforward and feedback delay channels\n";
	std::cout << "\"resetff\" to reset all feedforward delay channels\n";
	std::cout << "\"resetfb\" to reset all feedback delay channels\n";
	std::cout << "\"setdryamp:<number>\" set the amplitude for the dry input signal in the delay mix (does not affect feedforward delay)\n";
	std::cout << "\"setoutamp:<number>\" set the mix output amplitude.\n";
...
void AudioPB::cmdui_print_current_params(void)
{
	...
	std::cout << "Dry signal amplitude: " << std::to_string(f_amp) << std::endl;

	f_amp = this->p_delay->getOutputAmplitude();
	std::cout << "Output amplitude: " << std::to_string(f_amp) << "\n\n";
...
/*
 * cmdui_attempt_update_dryamp() & cmdui_attempt_update_outamp()
 *
 * Attempts to update the dry input amplitude / output amplitude.
 *
 * Inputs: 
 * "cmdarg" the const char* string with the amplitude value.
 *
 * returns true if successful, false otherwise.
 */

bool AudioPB::cmdui_attempt_update_dryamp(const char *cmdarg)
{
	float f_amp = 0.0f;

	try
	{
		f_amp = std::stof(cmdarg);
	}
	catch(...)
	{
...
bool AudioPB::cmdui_attempt_update_outamp(const char *cmdarg)
{
	float f_amp = 0.0f;
 ...
 *
 * The following methods, with exception of "cmdui_attempt_update_dryamp()" and "cmdui_attempt_update_outamp()"
 *
