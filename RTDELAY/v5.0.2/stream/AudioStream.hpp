...
class AudioStream {
	public:
		...
		bool chooseDevice(const char *name, bool enable_resampling, bool output);
		bool chooseDevice(size_t index, bool enable_resampling, bool output);
		bool chooseDefaultDevice(bool enable_resampling, bool output);
	...
	protected:
		...
		__attribute__((__aligned__(PTR_SIZE_BITS))) audiodevice_t audiodev_in = {
			.p_device = NULL,
			.device_name = "",
			.stream_mode = -1,
			.nonblock = 0,
		...
		__attribute__((__aligned__(PTR_SIZE_BITS))) audiodevice_t audiodev_out = {
			.p_device = NULL,
			.device_name = "",
			.stream_mode = -1,
			.nonblock = 0,
		...
		bool _choose_device(const char *name, bool enable_resampling, bool output);

		bool audio_hw_init(audiodevice_t *p_audiodev);
		void audio_hw_deinit(audiodevice_t *p_audiodev);
