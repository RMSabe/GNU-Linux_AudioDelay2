...
int main(int argc, char **argv)
{
	...
	delay_params.audio_dev_desc = argv[1];
	delay_params.file_dir = argv[2];

	delay_params.buffer_size_frames = RTDELAY_BUFFER_SIZE_FRAMES;
	delay_params.n_ff_delays = RTDELAY_N_FF_DELAYS;
	delay_params.n_fb_delays = RTDELAY_N_FB_DELAYS;
	...
	{
		case PB_I16:
			p_audio = new AudioRTDelay_i16(&delay_params);
			break;

		case PB_I24:
			p_audio = new AudioRTDelay_i24(&delay_params);
			break;
	}
...
bool filein_check_ext(void)
{
	...
	if(delay_params.file_dir == NULL) return false;

	snprintf(textbuf, TEXTBUF_SIZE_CHARS, "%s", delay_params.file_dir);
...
bool filein_open(void)
{
	...
	h_filein = open(delay_params.file_dir, O_RDONLY);
...
int filein_get_params(void)
{
	...
	delay_params.n_channels = p_u16[1];

	p_u32 = (uint32_t*) &p_headerinfo[bytepos + 12u];

	delay_params.sample_rate = *p_u32;
	...
	delay_params.audio_data_begin = (__offset) (bytepos + 8u);
	delay_params.audio_data_end = delay_params.audio_data_begin + ((__offset) *p_u32);
