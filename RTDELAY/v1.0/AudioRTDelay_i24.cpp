...
#include "AudioRTDelay_i24.hpp"
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <iostream>

AudioRTDelay_i24::AudioRTDelay_i24(const audiortdelay_init_params_t *p_params) : AudioRTDelay(p_params)
{
}

AudioRTDelay_i24::~AudioRTDelay_i24(void)
{
...
bool AudioRTDelay_i24::audio_hw_init(void)
{
	...
	if(n_ret < 0)
	{
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to open audio device.";
		return false;
	}
	...
	if(n_ret < 0)
	{
		...
		this->audio_hw_deinit();
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to set hardware access.";
	...
	if(n_ret < 0)
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to set format.";
	...
	if(n_ret < 0)
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to set number of channels.";
	...
	if((n_ret < 0) || (rate < this->SAMPLE_RATE))
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to set sample rate.";
	...
	if((n_ret < 0) || (n_frames != ((snd_pcm_uframes_t) this->AUDIOBUFFER_SIZE_FRAMES_DEFAULT)))
		std::cout << "AudioRTDelay_i24::audio_hw_init: Warning: could not set audio buffer size to default value.\n";
	...
	if(n_ret < 0)
	{
		snd_pcm_hw_params_free(p_hwparams);
		this->audio_hw_deinit();
		this->err_msg = "AudioRTDelay_i24::audio_hw_init: Error: failed to validate device settings.";
	...
	this->BUFFER_SIZE_BYTES = (this->BUFFER_SIZE_SAMPLES)*4u;
...
bool AudioRTDelay_i24::buffer_alloc(void)
{
	size_t n_seg = 0u;
...
void AudioRTDelay_i24::buffer_free(void)
{
...
void AudioRTDelay_i24::buffer_load(void)
{
	size_t n_sample = 0u;
	size_t n_byte = 0u;
	int32_t i32 = 0;
	...
	{
		i32 = ((this->p_bytebuf[n_byte + 2u] << 16) | (this->p_bytebuf[n_byte + 1u] << 8) | (this->p_bytebuf[n_byte]));

		if(i32 & 0x00800000) i32 |= 0xff800000;
		else i32 &= 0x007fffff; /*Not really necessary, but just to be safe.*/

		((int32_t**) this->pp_bufferinput_segments)[this->n_loadsegment][n_sample] = i32;
...
void AudioRTDelay_i24::run_dsp(void)
{
	...
	for(n_frame = 0u; n_frame < this->BUFFER_SEGMENT_SIZE_FRAMES; n_frame++)
	{
		...
		{
			p_samples[n_channel] = (float) ((int32_t**) this->pp_bufferinput_segments)[this->n_loadsegment][n_currsample];
			n_currsample++;
		}
		...
		while(n_fx < this->P_FF_PARAMS_LENGTH)
		{
			...
			for(n_channel = 0u; n_channel < this->N_CHANNELS; n_channel++)
			{
				p_samples[n_channel] += f_amp*((float) ((int32_t*) this->p_bufferinput)[n_prevsample]);
				n_prevsample++;
			}
		...
		while(n_fx < this->P_FB_PARAMS_LENGTH)
		{
			...
			for(n_channel = 0u; n_channel < this->N_CHANNELS; n_channel++)
			{
				p_samples[n_channel] += f_amp*((float) ((int32_t*) this->p_bufferoutput)[n_prevsample]);
				n_prevsample++;
			}
		...
		for(n_channel = 0u; n_channel < this->N_CHANNELS; n_channel++)
		{
			if(p_samples[n_channel] >= this->SAMPLE_MAX_VALUE_F) ((int32_t**) this->pp_bufferoutput_segments)[this->n_loadsegment][n_currsample] = this->SAMPLE_MAX_VALUE;
			else if(p_samples[n_channel] <= this->SAMPLE_MIN_VALUE_F) ((int32_t**) this->pp_bufferoutput_segments)[this->n_loadsegment][n_currsample] = this->SAMPLE_MIN_VALUE;
			else ((int32_t**) this->pp_bufferoutput_segments)[this->n_loadsegment][n_currsample] = (int32_t) roundf(p_samples[n_channel]);
