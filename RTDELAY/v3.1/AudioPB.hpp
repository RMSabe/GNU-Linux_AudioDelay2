...
class AudioPB {
	...
	protected:
		...
		__attribute__((__aligned__(PTR_SIZE_BITS))) __string user_cmd = __TEXT("");

		__attribute__((__aligned__(PTR_SIZE_BITS))) std::thread playthread;
		__attribute__((__aligned__(PTR_SIZE_BITS))) std::thread userthread;
		...
		ssize_t cmdui_parse_nfx(const __tchar_t *p_text_nfx, ssize_t max);
		ssize_t cmdui_parse_ndelay(const __tchar_t *p_text_ndelay);
		bool cmdui_parse_famp(const __tchar_t *p_text_famp, float *p_famp);
