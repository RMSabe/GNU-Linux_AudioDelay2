...
bool loadfile_createaudioobject(void)
{
	...
	pb_params.streambuffer_n_segments = __AUDIO_STREAMBUFFER_N_SEGMENTS;
	pb_params.delay_buffer_size_frames = __AUDIO_DELAY_BUFFER_SIZE_FRAMES;
	pb_params.delay_n_ff_delays = __AUDIO_DELAY_N_FFCH;
	pb_params.delay_n_fb_delays = __AUDIO_DELAY_N_FBCH;
...
bool choose_device(size_t index, bool defaultdev, bool enable_resampling)
{
	bool b_ret;
	...
	if(defaultdev) b_ret = p_audio->chooseDefaultDevice(enable_resampling);
	else b_ret = p_audio->chooseDevice(index, enable_resampling);
...
__offset_t audiodata_get_size_frames(void)
{
	__offset_t _noff;

	if(p_audio == NULL)
	{
		err_msg = __TEXT("audiodata_get_size_frames: Error: no audio object instance.");
		return -1;
	}

	_noff = p_audio->getAudioDataSizeFrames();
	if(_noff < 0) err_msg = p_audio->getLastErrorMessage();

	return _noff;
}

__offset_t audiodata_get_position_frames(void)
{
	__offset_t _noff;

	if(p_audio == NULL)
	{
		err_msg = __TEXT("audiodata_get_position_frames: Error: no audio object instance.");
		return -1;
	}

	_noff = p_audio->getAudioDataPositionFrames();
	if(_noff < 0) err_msg = p_audio->getLastErrorMessage();

	return _noff;
}

bool audiodata_set_position_frames(__offset_t position)
{
	if(p_audio == NULL)
	{
		err_msg = __TEXT("audiodata_set_position_frames: Error: no audio object instance.");
		return false;
	}

	if(position < 0)
	{
		err_msg = __TEXT("audiodata_set_position_frames: Error: invalid position given.");
		return false;
	}

	if(!p_audio->setAudioDataPositionFrames(position))
	{
		err_msg = p_audio->getLastErrorMessage();
		return false;
	}
