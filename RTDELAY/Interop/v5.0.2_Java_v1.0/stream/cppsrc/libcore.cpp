...
bool choose_device(size_t index, bool defaultdev, bool enable_resampling, bool output)
{
	bool b_ret;
	...
	if(defaultdev) b_ret = p_audio->chooseDefaultDevice(enable_resampling, output);
	else b_ret = p_audio->chooseDevice(index, enable_resampling, output);
