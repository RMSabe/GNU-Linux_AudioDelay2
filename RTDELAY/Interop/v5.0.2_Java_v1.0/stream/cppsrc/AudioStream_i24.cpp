...
AudioStream_i24::AudioStream_i24(const audiostream_params_t *p_params) : AudioStream(p_params)
{
	this->AUDIO_BYTES_PER_SAMPLE = 4u;
	this->AUDIODEV_FORMAT = SND_PCM_FORMAT_S24_LE;
}

AudioStream_i24::~AudioStream_i24(void)
{
	this->deinitialize();
}

void AudioStream_i24::delaybuffer_loadin(void)
{
	size_t n_sample = 0u;
	...
	p_input = (int32_t*) (((uintptr_t) (this->p_streambuffer_in)) + n_inputseg*(this->STREAMBUFFER_SEGMENT_SIZE_BYTES));

	factor = this->SAMPLE_FACTOR;

	for(n_sample = 0u; n_sample < this->STREAMBUFFER_SEGMENT_SIZE_SAMPLES; n_sample++)
	{
		/*if(p_input[n_sample] & 0x00800000) p_input[n_sample] |= 0xff800000;*/ 
...
void AudioStream_i24::delaybuffer_loadout(void)
{
	size_t n_sample = 0u;
	...
	p_output = (int32_t*) (((uintptr_t) (this->p_streambuffer_out)) + n_outputseg*(this->STREAMBUFFER_SEGMENT_SIZE_BYTES));
