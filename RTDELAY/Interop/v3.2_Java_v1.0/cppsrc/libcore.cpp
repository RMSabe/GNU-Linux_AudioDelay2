...
bool loadfile_createaudioobject(procctx_t *p_procctx)
{
	int n_ret;
	...
	{
		*(p_procctx->p_errmsg) = __TEXT("core_loadfile_createaudioobject: Error: could not open input file.");
		return false;
	}
	...
	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_loadfile_createaudioobject: Error: audio object instance failed.");
		return false;
	}
...
bool load_audiodevicelist(procctx_t *p_procctx)
{
	if(p_procctx == NULL) return false;

	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_load_audiodevicelist: Error: no audio object instance.");
		return false;
	}
...
ssize_t audiodevicelist_get_entry_count(procctx_t *p_procctx)
{
	ssize_t _ssize;
	...
	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_audiodevicelist_get_entry_count: Error: no audio object instance.");
		return -1;
	}

	_ssize = p_procctx->p_audio->getAudioDeviceListEntryCount();
	if(_ssize < 0) *(p_procctx->p_errmsg) = p_procctx->p_audio->getLastErrorMessage();

	return _ssize;
}

const char* audiodevicelist_get_entry_info(procctx_t *p_procctx, size_t index, bool info)
{
	...
	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_audiodevicelist_get_entry_info: Error: no audio object instance.");
		return NULL;
	}

	p_entry = p_procctx->p_audio->getAudioDeviceListEntry(index);
	if(p_entry == NULL)
	{
		*(p_procctx->p_errmsg) = p_procctx->p_audio->getLastErrorMessage();
		return NULL;
...
bool choose_device(procctx_t *p_procctx, size_t index, bool defaultdev)
{
	bool b_ret;
	...
	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_choose_device: Error: no audio object instance.");
		return false;
	}

	if(defaultdev) b_ret = p_procctx->p_audio->chooseDefaultDevice();
	else b_ret = p_procctx->p_audio->chooseDevice(index);

	if(!b_ret) *(p_procctx->p_errmsg) = p_procctx->p_audio->getLastErrorMessage();

	return b_ret;
}

bool audioobject_init(procctx_t *p_procctx)
{
	if(p_procctx == NULL) return false;

	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_audioobject_init: Error: no audio object instance.");
		return false;
	}
...
bool run_playback(procctx_t *p_procctx)
{
	if(p_procctx == NULL) return false;

	if(p_procctx->p_audio == NULL)
	{
		*(p_procctx->p_errmsg) = __TEXT("core_run_playback: Error: no audio object instance.");
		return false;
	}
