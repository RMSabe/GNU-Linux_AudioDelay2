...
bool AudioRTDelay::setInitParameters(const audiortdelay_init_params_t *p_params)
{
	...
	if(p_params == NULL)
	{
		this->err_msg = __TEXT("AudioRTDelay::setInitParameters: Error: init params object pointer is null.");
		return false;
	}
...
bool AudioRTDelay::initialize(void)
{
	...
	{
		this->err_msg = __TEXT("AudioRTDelay::initialize: Error: BUFFER SIZE FRAMES value is invalid.");
		return false;
	}

	if(this->BUFFER_N_SEGMENTS < this->BUFFER_N_SEGMENTS_MIN)
	{
		this->err_msg = __TEXT("AudioRTDelay::initialize: Error: BUFFER N SEGMENTS value is invalid.");
		return false;
	}

	if(this->N_CHANNELS < this->N_CHANNELS_MIN)
	{
		this->err_msg = __TEXT("AudioRTDelay::initialize: Error: N CHANNELS value is invalid.");
		return false;
	}
	...
	if(!this->buffer_alloc())
	{
		...
		this->err_msg = __TEXT("AudioRTDelay::initialize: Error: memory allocate failed.");
		return false;
	}
...
bool AudioRTDelay::runDSP(size_t n_segment)
{
	...
	{
		this->err_msg = __TEXT("AudioRTDelay::runDSP: Error: given segment index is out of bounds.");
		return false;
	}
...
float* AudioRTDelay::getInputBufferSegment(size_t n_segment)
{
	...
	if(n_segment >= this->BUFFER_N_SEGMENTS)
	{
		this->err_msg = __TEXT("AudioRTDelay::getInputBufferSegment: Error: given segment index is out of bounds.");
		return NULL;
	}
...
float* AudioRTDelay::getOutputBufferSegment(size_t n_segment)
{
	...
	if(n_segment >= this->BUFFER_N_SEGMENTS)
	{
		this->err_msg = __TEXT("AudioRTDelay::getOutputBufferSegment: Error: given segment index is out of bounds.");
		return NULL;
	}
...
bool AudioRTDelay::getFFParams(size_t n_fx, audiortdelay_fx_params_t *p_params)
{
	...
	{
		this->err_msg = __TEXT("AudioRTDelay::getFFParams: Error: given fx index is out of bounds.");
		return false;
	}

	if(p_params == NULL)
	{
		this->err_msg = __TEXT("AudioRTDelay::getFFParams: Error: given params object pointer is null.");
		return false;
	}
...
bool AudioRTDelay::getFBParams(size_t n_fx, audiortdelay_fx_params_t *p_params)
{
	...
	{
		this->err_msg = __TEXT("AudioRTDelay::getFBParams: Error: given fx index is out of bounds.");
		return false;
	}

	if(p_params == NULL)
	{
		this->err_msg = __TEXT("AudioRTDelay::getFBParams: Error: given params object pointer is null.");
		return false;
	}
...
bool AudioRTDelay::setFFDelay(size_t n_fx, uint32_t delay)
{
	...
	if(n_fx >= this->P_FF_PARAMS_LENGTH)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFFDelay: Error: given fx index is out of bounds.");
		return false;
	}

	if(((size_t) delay) >= this->BUFFER_SIZE_FRAMES)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFFDelay: Error: given delay value is too big.");
		return false;
	}
...
bool AudioRTDelay::setFFAmplitude(size_t n_fx, float amp)
{
	...
	if(n_fx >= this->P_FF_PARAMS_LENGTH)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFFAmplitude: Error: given fx index is out of bounds.");
		return false;
	}
...
bool AudioRTDelay::setFBDelay(size_t n_fx, uint32_t delay)
{
	...
	if(n_fx >= this->P_FB_PARAMS_LENGTH)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFBDelay: Error: given fx index is out of bounds.");
		return false;
	}

	if(((size_t) delay) >= this->BUFFER_SIZE_FRAMES)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFBDelay: Error: given delay value is too big.");
		return false;
	}
...
bool AudioRTDelay::setFBAmplitude(size_t n_fx, float amp)
{
	...
	if(n_fx >= this->P_FB_PARAMS_LENGTH)
	{
		this->err_msg = __TEXT("AudioRTDelay::setFBAmplitude: Error: given fx index is out of bounds.");
		return false;
	}
...
__string AudioRTDelay::getLastErrorMessage(void)
{
	if(this->status == this->STATUS_UNINITIALIZED)
		return __TEXT("AudioRTDelay object not initialized\nExtended error message: ") + this->err_msg;

	return this->err_msg;
}

void AudioRTDelay::deinitialize(void)
{
	this->status = this->STATUS_UNINITIALIZED;
	this->buffer_free();
...
bool AudioRTDelay::buffer_alloc(void)
{
	...
	{
		this->pp_bufferinput_segments[n_seg] = (float*) (((uintptr_t) this->p_bufferinput) + n_seg*(this->BUFFER_SEGMENT_SIZE_BYTES));
		this->pp_bufferoutput_segments[n_seg] = (float*) (((uintptr_t) this->p_bufferoutput) + n_seg*(this->BUFFER_SEGMENT_SIZE_BYTES));
	}
