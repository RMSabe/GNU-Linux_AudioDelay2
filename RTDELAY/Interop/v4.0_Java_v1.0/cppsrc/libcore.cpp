...
ssize_t rtdelay_getFFDelay(size_t n_fx)
{
	...
	_ndelay = p_audio->rtdelayGetFFDelay(n_fx);
	if(_ndelay < 0)
	{
		err_msg = p_audio->getLastErrorMessage();
		return -1;
...
float rtdelay_getFFAmplitude(size_t n_fx)
{
	...
	return p_audio->rtdelayGetFFAmplitude(n_fx);
}
...
ssize_t rtdelay_getFBDelay(size_t n_fx)
{
	...
	_ndelay = p_audio->rtdelayGetFBDelay(n_fx);
	if(_ndelay < 0)
	{
		err_msg = p_audio->getLastErrorMessage();
		return -1;
...
float rtdelay_getFBAmplitude(size_t n_fx)
{
	...
	return p_audio->rtdelayGetFBAmplitude(n_fx);
}
